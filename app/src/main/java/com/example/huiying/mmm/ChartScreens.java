...
public class ChartScreens extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        db.execSQL("CREATE TABLE IF NOT EXISTS account" +
                "(id integer primary key, category VARCHAR, amount VARCHAR);");
        ...
        Cursor c = db.rawQuery("SELECT distinct category FROM account", null);
        int cateColumn =c.getColumnIndex("category");
        c.moveToFirst();
        String cate;
       ...
       for (int i = 1; i<cateEntry.size();i++) {
        ...
        for(int n = 0; n<category.size(); n++){
            Cursor cExp = db.rawQuery(
                    "SELECT SUM (amount) as Total FROM account where category ='" + category.get(n) + "'", null);
